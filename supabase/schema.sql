...
-- Runs on Supabase (Postgres) AND embedded PGlite (Postgres/WASM).
-- ============================================================

-- Clean slate (safe to re-run locally)
...
CREATE TABLE hierarchy_levels (
    ...
    created_at TIMESTAMPTZ DEFAULT NOW()
    -- NOTE: the Setup Guide schema declared UNIQUE(org_id, level_number, department)
    -- here, but its own seed data violates it (three Level-8 Ortho units all map to
    -- (supra, 8, 'ortho')). The primary key on `id` already guarantees uniqueness,
    -- so we drop the broken composite constraint. Documented in docs/architecture.md.
);
...
-- Indexes for performance (checks 1-4 execute as indexed SQL WHERE clauses)
